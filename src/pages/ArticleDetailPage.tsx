...
import { ArrowLeft, CalendarDays, Edit2, MessageCircle, Trash2 } from 'lucide-react'
...
const ArticleDetailPage = () => {
  ...
  const { data: commentList } = useGetCommentList({
    params: {
  ...
  const handleEditComment = (documentId: string) => {
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="space-x-2">
            <Button variant="destructive" size="sm" onClick={() => handleDeleteArticle(documentId as string)}>
        ...
        <article>
          <header className="mb-8">
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">{articleDetail.data.title}</h1>
            <div className="flex items-center text-sm text-muted-foreground space-x-4">
              <span className="flex items-center"><CalendarDays className="mr-1.5 h-4 w-4" /> Published on {new Date(articleDetail.data.createdAt).toLocaleDateString()}</span>
            </div>
          </header>
        ...
        <section className="mt-12 pt-8 border-t">
          <h2 className="text-2xl font-semibold mb-6 flex items-center">
            <MessageCircle className="mr-3 h-6 w-6 text-primary" /> Comments ({commentList?.data.filter((filterItem) => filterItem.user.id === userAuth?.id)?.length})
          </h2>
            ...
            <CardContent>
              <form className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea id="commentText" placeholder="Write your comment here..." required rows={4} />
                </div>
          ...
          <div className="space-y-6">
            {commentList?.data.filter((filterItem) => filterItem.user.id === userAuth?.id).map(item => (
              <Card key={item.id} className="shadow-sm">
                <CardContent className="p-4">
                  <div className="mb-6 flex justify-end items-center gap-2">
                    <div className="space-x-2">
                      <Button size="sm" onClick={() => handleEditComment(item.documentId)}>
                        <Edit2 className="mr-2 h-4 w-4" /> Edit
                      </Button>
                    </div>
                    <div className="space-x-2">
                      <Button variant="destructive" size="sm" onClick={() => handleDeleteComment(item.documentId)}>
                        <Trash2 className="mr-2 h-4 w-4" /> Delete
                      </Button>
                    </div>
                  </div>
                  <div className="flex justify-between items-center mb-1">
                    <p className="font-semibold text-sm text-primary">{item?.user?.username}</p>
                    <p className="text-xs text-muted-foreground">{new Date(item.createdAt).toLocaleDateString()}</p>
                  </div>
                  <p className="text-sm text-left text-primary-foreground">{item.content}</p>
                </CardContent>
              </Card>
            ))}
